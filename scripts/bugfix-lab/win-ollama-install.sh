...
if (installed) {
  console.log("BUG ABSENT: the install actually happened.");
  console.log("BUGFIX_LAB_RESULT=BUGFIX_LAB_ABSENT"); process.exit(0);
}
console.log("BUG PRESENT: after " + r.clicks + " clicks on Install, no ollama binary exists anywhere findBinary() looks, nothing is serving on 11434, and the row is not Connected -- the button reverted to idle Install and Ollama was never installed.");
